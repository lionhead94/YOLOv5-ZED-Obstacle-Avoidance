...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw) {
    if (argc < 3) 
        return false;
    if (argc == 4 || argc == 6) {
        wts = std::string(argv[1]);
        engine = std::string(argv[2]);
        auto net = std::string(argv[3]);
        if (net[0] == 'n') {
            gd = 0.33;
            gw = 0.25;
        } else if (net[0] == 's') {
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw)) {
        ...
        std::cerr << "./yolov5_ser [.wts] [.engine] [n/s/m/l/x/n6/s6/m6/l6/x6 or c/c6 gd gw]  // serialize model to plan file" << std::endl;
        return -1;
    }
